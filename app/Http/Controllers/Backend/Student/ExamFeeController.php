...
class ExamFeeController extends Controller
{
    ...
    public function getStudent(Request $request)
    {
    	...
    	foreach($allStudent as $key => $v){
    		$registrationfee = FeeCategoryAmount::where('fee_category_id','3')->where('department_id',$v->department_id)->first();
    		$color = 'success';
    		...
    		$html[$key]['tdsource'].='<a class="btn btn-sm btn-'.$color.'"title="Payslip" target="_blank" href="'.route("students.exam.fee.payslip").'?department_id='.$v->department_id.'&student_id='.$v->student_id.'exam_type_id='.$request->exam_type_id.'">Fee Slip</a>';
    		$html[$key]['tdsource'].='<td>';
    ...
    public function paySlip(Request $request){
        ...
        $department_id = $request->department_id;
        $data['exam_type_id'] = ExamTypes::where('id',$request->exam_type_id)->first()['name'];
        ...
        $pdf = PDF::loadView('backend.student.exam_fee.exam-fee-pdf',$allStudent);
